...
namespace forza_saveswapper {
    ...
    CEncryption<Rounds>::CEncryption(
    ...
    template<size_t Rounds>
    void CEncryption<Rounds>::RoundB(
    ...
    template<size_t Rounds>
    void CEncryption<Rounds>::Update(std::array<uint8_t, 16> &src, std::array<uint8_t, 16> &dst) {
        std::transform(
            std::begin(src),
            std::end(src),
            std::begin(this->previous_),
            std::begin(src),
            std::bit_xor<uint8_t>()
        );
    ...
    template class CEncryption<17>;
    template class CEncryption<13>;
